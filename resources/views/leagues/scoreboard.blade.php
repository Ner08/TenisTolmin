<x-layout>
    ...
    @else
        <section class="pt-4 pb-8 md:py-8 xl:py-12 px-4">
            <div class="container mx-auto max-w-4xl xl:max-w-5xl">
                <div class="space-y-3">
                    @foreach ($players as $key => $player)
                        ...
                        <a href="{{ route('player.show', $player->id) }}" class="bg-white border border-gray-200 rounded-xl overflow-hidden flex items-stretch shadow-sm hover:shadow-md transition-shadow duration-200 group">
                            {{-- Rank badge --}}
                            <div @class([
                                'w-16 xl:w-20 flex flex-col items-center justify-center text-center flex-shrink-0 py-4 xl:py-5',
                                ...
                                @endif
                                <span class="text-xl xl:text-2xl font-bold {{ $ranking <= 3 ? 'text-gray-800' : 'text-gray-500' }}">{{ $ranking }}</span>
                            </div>
                            {{-- Player info --}}
                            <div class="flex-grow px-5 xl:px-7 py-4 xl:py-5">
                                <div class="flex items-center justify-between mb-3">
                                    <span class="font-semibold xl:text-lg text-gray-900 group-hover:text-amber-600 transition-colors">{{ $player->p_name }}</span>
                                    <span class="text-sm xl:text-base font-semibold text-amber-600 bg-amber-50 border border-amber-200 px-3 py-0.5 rounded-full">
                                        {{ $player->points }} točk
                                    </span>
                                </div>
